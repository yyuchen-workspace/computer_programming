...
void if_hit_the_wall(int *move, unsigned char computer_x, unsigned char computer_y)
{
    ...
    if (new_x > 0 && new_x < map_size - 1 && new_y > 0 && new_y < map_size - 1)
    {
        valid_move = true;
    }
    while(!valid_move)
...
int choose_move_toward_food(int *move, unsigned char computer_x, unsigned char computer_y, unsigned char fx, unsigned char fy) {
                   ...
                   unsigned char *computer_length, unsigned char map[map_size][map_size])
{
    ...
    *move = choose_move_toward_food(move, computer_x, computer_y, fx, fy);
    if_hit_the_wall(move, computer_x, computer_y);  // 確保不撞牆或障礙
...
void move_lazer(const unsigned char *snake, unsigned char *lazer, int *lazer_time,
                const unsigned char *length, const unsigned char *computer_length, unsigned char *lazer_length, unsigned char map[map_size][map_size])
{
    if (*lazer_time == 5)
    {
        ...
        do
        {
            ...
            for (int i = 1; i < *lazer_length; i++)
            {
                ...
                if (lazer_x >= map_size || lazer_y >= map_size || map[lazer_y][lazer_x] != 0 || (lazer_x == x && lazer_y == y))
                {
                    OK = false;
                    break;
                }
...
void hit_the_snake_or_lazer(unsigned char *snake, unsigned char *computer, unsigned char *length, unsigned char *computer_length, unsigned char map[map_size][map_size], bool *win, bool *lose)
{
...
void map_setting(const unsigned char *snake, const unsigned char *computer, const unsigned char *lazer,
                 unsigned char *food,
                 const unsigned char *length, const unsigned char *computer_length, const unsigned char *lazer_length, unsigned char map[map_size][map_size])
{
...
void move_all(int direct, int *move, unsigned char *food, unsigned char *head_x, unsigned char *head_y,//fx, fy = food x, y座標 //x, y 蛇座標
                unsigned char *snake, unsigned char *computer, unsigned char *lazer, int *lazer_time, unsigned char *length, unsigned char *computer_length,  unsigned char *lazer_length, unsigned char map[map_size][map_size], bool *win, bool *lose)
{
	unsigned char fx, fy;//fx, fy = food x, y座標 //x, y 蛇座標
    fx = *food >> 4;//利用二進位 右移4位計算x值
	fy = *food & 0x0F;//利用&運算符計算y值（十六進位 0x0F = 二進位 0000 1111）

    move_lazer(snake, lazer, lazer_time, length, computer_length, lazer_length, map);
    move_snake(direct, food, head_x, head_y, fx, fy, snake, computer, length, computer_length, map);
    move_computer(move, food, fx, fy, snake, computer, length, computer_length, map);
    hit_the_snake_or_lazer(snake, computer, length, computer_length, map, win, lose);
	map_setting(snake, computer, lazer, food, length, computer_length, lazer_length, map);
...
int main()
{
    ...
    unsigned char lazer[lazer_size] = {0};
	...
	while(1)
	{
		...
		move_all(direct, &move, &food, &head_x, &head_y, snake, computer, lazer, &lazer_time, &length, &computer_length, &lazer_length, map, &win, &lose);
