...
{
    int spiral_B[map_size][map_size], spiral_G[map_size][map_size];
    int mid = size / 2;
    int row = 0, column = 0;
    int R = 0, G = 255, B = 255;//開始點
    spiral_B[mid][mid] = B;
    spiral_G[mid][mid] = G;
    ...
    for(int i = 0 ; i < size ; i++)
    {
        for(int j = 0 ; j < size ; j++)
        {
            printf("\033[48;2;0;%d;%dm  \033[0m",  spiral_G[i][j], spiral_B[i][j]);
        }
        printf("\n");
    }
